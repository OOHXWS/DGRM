...
def TT(dataset, reliability_matrix, model, purchase_matrix, mask_matrix, test_uis_dict,tt_test_mask_matrix, topk=5):
    mean_type = 'x0'
    diffusion = gd.GaussianDiffusion(mean_type, args.noise_schedule, args.noise_scale, args.noise_min, args.noise_max,args.steps+1 , device='cpu')
    ...
    with torch.no_grad():
        for idxs in dataloader_tt:
            ...
            for i in reversed(range(2)):
                ...
                v += 1
            out = x.numpy()
            top_k, recall_u, precisions_u, ndcg_u, hit, rec = recall_precision_f1_hr_ndcg(out, 0, out, mask_matrix[idxs],tt_test_mask_matrix,test_uis_dict_u, top_k=topk)
...
def train_DGRM(save_dir, purchase_matrix,interact_matrix, mask_matrix, train_set, test_uis_dict, tt_test_mask_matrix, nb_item, epoches, batch_size, nb_zr, nb_pm, alpha, top_k):

    save_path = os.path.join(save_dir, 'model_1.pkl')
    early_stop = EarlyStopper(num_trials=128, save_path=save_path)
    ...
    for e in range(epoches):
        ...
        for step in range(step_dis):
            for idxs in dataloader:
                ...
                real_vec, real_vec_1 = diffusion.q_sample_pair(x_start,t)#, condition_vec
                ...
                grad_real = torch.autograd.grad(
                    outputs=D_real.sum(), inputs=real_vec, create_graph=True
                )[0]
                grad_penalty = (
        ...
        for step in range(step_gen):
            for idxs in dataloader:
                ...
                D_fake = dis_ra(real_vec_1.detach(), fake_vec, t)
                loss_fake = D_fake.sum()
                loss_regular = alpha*torch.sum(((fake_x_start-x_start)* eu).pow(2))
...
if __name__=='__main__':
    ...
    train_DGRM(save_dir='models/ml100k', purchase_matrix=train_score_matrix, interact_matrix=train_mask_matrix,
               ...
               tt_test_mask_matrix=test_mask_matrix, nb_item=nb_item, epoches=50000, batch_size=128, nb_zr=100,
               nb_pm=30, alpha=0.1, top_k=5)
