...
class GaussianDiffusion(nn.Module):
    ...
    def var_func_vp(self, t, beta_min, beta_max):
    ...
    def calculate_for_diffusion(self):
        ...
        self.alphas_cumprod_prev = th.cat([th.tensor([1.0],dtype=float), self.alphas_cumprod[:-1]]).to(self.device)  # alpha_{t-1}
        self.alphas_cumprod_next = th.cat([self.alphas_cumprod[1:], th.tensor([0.0],dtype=float).to(self.device)]).to(self.device)  # alpha_{t+1}
    ...
    def sample_posterior(self, x_0, x_t, t):
        ...
        def q_posterior(x_0, x_t, t):
            mean = (
                    self._extract_into_tensor(self.posterior_mean_coef1, t, x_t.shape) * x_0
                    + self._extract_into_tensor(self.posterior_mean_coef2, t, x_t.shape) * x_t
            )
            var = self._extract_into_tensor(self.posterior_variance, t, x_t.shape)
            log_var_clipped = self._extract_into_tensor(self.posterior_log_variance_clipped, t, x_t.shape)
        ...
        def p_sample(x_0, x_t, t):
            ...
            return mean + nonzero_mask * th.exp(0.5 * log_var) * noise

        sample_x_pos = p_sample(x_0, x_t, t)

        return sample_x_pos

    def p_sample(self, model, x_start, steps, sampling_noise=True):
    ...
    def sample_timesteps(self, T, batch_size):
            t = th.randint(0, T, (batch_size,)).long()
            return t
    ...
    def q_sample_pair(self,x_start, t):
        noise = th.randn_like(x_start)
        assert noise.shape == x_start.shape

        # condition_t = th.round(t/4)
        # x_0_condition = self.q_sample(x_start, condition_t.long(), noise)
        x_t = self.q_sample(x_start, t, noise)
        x_p_1 = self._extract_into_tensor(self.sqrt_alphas_cumprod, t + 1, x_start.shape) * x_t + self._extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t + 1, x_start.shape) * noise
